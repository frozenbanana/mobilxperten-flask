...
def get_device(id):
    device = RepairDevice.query.get_or_404(id, description='There is no data with id {}'.format(id))
...
def get_devices():
    devices = RepairDevice.query.limit(100).all()
...
def update_device(id):
    device = RepairDevice.query.get_or_404(id)
    data = request.get_json() or {}
    if 'model' in data and data['model'] != device.model and \
            RepairDevice.query.filter_by(model=data['model']).first():
